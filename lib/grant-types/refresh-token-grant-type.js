...
class RefreshTokenGrantType extends AbstractGrantType {
  ...
  async handle(request, client) {
    ...
    return this.saveToken(token, client, scope);
  }
  ...
  async saveToken(token, client, scope) {
    const accessToken = await this.generateAccessToken(client, token.user, scope);
    const refreshToken = await this.generateRefreshToken(client, token.user, scope);
    ...
    return this.model.saveToken(token, client, token.user);
  }
